         ...
         <tbody>
            ...
            @foreach ($mutasi as $d)
               @php
                  ...
                  $jml_pengeluaran = $d->penjualan + $d->promosi + $d->reject_pasar + $d->reject_mobil + $d->reject_gudang + $d->transit_out + $jml_lainlain_out + $d->penyesuaian_out;

                  $saldo_akhir_jumlah = $saldo_akhir_jumlah + $jml_penerimaan - $jml_pengeluaran;
                  $saldo_akhir_jumlah = $saldo_akhir_jumlah < 0 ? $saldo_akhir_pcs * -1 : $saldo_akhir_jumlah;
                  $saldo_akhir_desimal = $saldo_akhir_jumlah / $d->isi_pcs_dus;
               ...
               <tr>
                  ...
                  <td class="right">{{ !empty($d->penyesuaian_in) ? formatAngkaDesimal($penyesuaian_in) : '' }}</td>
                  ...
                  <td class="right">{{ !empty($d->penyesuaian_out) ? formatAngkaDesimal($penyesuaian_out) : '' }}</td>
                  <td class="right {{ $color_sa }}">{{ !empty($saldo_akhir_jumlah) ? formatAngkaDesimal($saldo_akhir_desimal) : '' }}</td>
