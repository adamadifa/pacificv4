<form method="POST" action="{{ route('laporangudangcabang.cetakpersediaangs') }}" id="frmLaporangoodstok" target="_blank">
   ...
   @hasanyrole($roles_show_cabang)
      <div class="row">
         <div class="col">
            <x-select label="Pilih Cabang" name="kode_cabang_gs" :data="$cabang"
               key="kode_cabang" textShow="nama_cabang" upperCase="true"
               select2="select2Kodecabanggs" hideLabel="true" />
         </div>
      </div>
   @endrole
   <div class="row">
      <div class="col">
         <x-select label="Pilih Produk" name="kode_produk_gs" :data="$produk" key="kode_produk" textShow="nama_produk"
            select2="select2Kodeprodukgs" showKey="true" upperCase="true" hideLabel="true" />
      </div>
   </div>
   <div class="row">
...
@push('myscript')
   <script>
      $(function() {
         const select2Kodecabanggs = $('.select2Kodecabanggs');
         ...
         const select2Kodeprodukgs = $('.select2Kodeprodukgs');
         ...
         $("#frmLaporangoodstok").submit(function() {
            const kode_produk = $(this).find("#kode_produk_gs").val();
            const dari = $(this).find("#dari").val();
            const sampai = $(this).find("#sampai").val();
            const kode_cabang = $(this).find("#kode_cabang_gs").val();
