<form action="{{ route('kirimpusat.update', Crypt::encrypt($kirimpusat->no_mutasi)) }}" method="POST" id="formKirimpusat" autocomplete="off" aria-autocomplete="none">
   @csrf
   @method('PUT')
   <div class="row mb-2">
      <div class="col">
         <x-input-with-icon icon="ti ti-calendar" label="Tanggal" name="tanggal" datepicker="flatpickr-date" value="{{ $kirimpusat->tanggal }}" hideLabel="true" />
         @hasanyrole($roles_show_cabang)
            <x-select label="Pilih Cabang" name="kode_cabang" :data="$cabang" key="kode_cabang" textShow="nama_cabang"
               upperCase="true" select2="select2Kodecabang" selected="{{ $kirimpusat->kode_cabang }}" hideLabel="true" />
         @endrole
         <x-input-with-icon icon="ti ti-file-description" label="Keterangan" name="keterangan" value="{{ $kirimpusat->keterangan }}" hideLabel="true" />
      </div>
   </div>
   <div class="row mb-2">
      <div class="col">
...
<script>
   $(function() {
      ...
      $(document).on('submit', '#formKirimpusat', function(e) {
