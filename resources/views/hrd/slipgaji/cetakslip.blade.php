...
@foreach ($presensi as $d)
    ...
    @while (strtotime($tanggal_presensi) <= strtotime($end_date))
        ...
        @if (isset($d[$tanggal_presensi]))
            @php
                ...
                $jam_in = !empty($d[$tanggal_presensi]['jam_in']) ? date('Y-m-d H:i:s', strtotime($d[$tanggal_presensi]['jam_in'])) : 'Belum Absen';
                $jam_out = !empty($d[$tanggal_presensi]['jam_out']) ? date('Y-m-d H:i:s', strtotime($d[$tanggal_presensi]['jam_out'])) : 'Belum Absen';
                //Jadwal SPG
            ...
            @if ($d[$tanggal_presensi]['status'] == 'h')
                ...
                @php
                    ...
                    $potongan_jam_izin = 0;
                    $potongan_jam_pulangcepat = $d[$tanggal_presensi]['izin_pulang_direktur'] == '1' || ($tanggal_presensi >= '2026-03-21' && $pulangcepat['desimal'] < 1 && !empty($d[$tanggal_presensi]['kode_izin_pulang'])) ? 0 : $pulangcepat['desimal'];
                ...
                @if (!empty($d[$tanggal_presensi]['doc_sid']) || $d[$tanggal_presensi]['izin_sakit_direktur'] == '1')
                    @php
                        if (!empty($cekmarketingimpact)) {
                            ...
                            $potongan_jam_sakit = 0;
                            $keterangan = 'SID';
                            $potongan_jam_dirumahkan = !in_array($d['nik'], $privillage_karyawan) ? $total_jam_jadwal / 2 : 0;
                        } else {
    ...
    <style>
        ...
        .slip-info-table td.separator-col {
            width: 10px;
        }
    ...
    <div class="slip-wrapper">
        <div class="slip-container">
            ...
            <div class="salary-section">
                <div class="insentif-title">INSENTIF</div>
                @if ($d['kategori_jabatan'] == 'MJ')
                    <div class="salary-component">
                        <div class="salary-label">Ruang Lingkup</div>
                        <div class="salary-value">{{ rupiah($d['im_ruanglingkup']) }}</div>
                    </div>
                    <div class="salary-component">
                        <div class="salary-label">Penempatan</div>
                        <div class="salary-value">{{ rupiah($d['im_penempatan']) }}</div>
                    </div>
                    <div class="salary-component">
                        <div class="salary-label">Kinerja</div>
                        <div class="salary-value">{{ rupiah($d['im_kinerja']) }}</div>
                    </div>
                    <div class="salary-component">
                        <div class="salary-label">Kendaraan</div>
                        <div class="salary-value">{{ rupiah($d['im_kendaraan']) }}</div>
                    </div>
                @else
                    <div class="salary-component">
                        <div class="salary-label">Masa Kerja</div>
                        <div class="salary-value">{{ rupiah($d['iu_masakerja']) }}</div>
                    </div>
                    <div class="salary-component">
                        <div class="salary-label">Lembur</div>
                        <div class="salary-value">{{ rupiah($d['iu_lembur']) }}</div>
                    </div>
                    <div class="salary-component">
                        <div class="salary-label">Penempatan</div>
                        <div class="salary-value">{{ rupiah($d['iu_penempatan']) }}</div>
                    </div>
                    <div class="salary-component">
                        <div class="salary-label">KPI</div>
                        <div class="salary-value">{{ rupiah($d['iu_kpi']) }}</div>
                    </div>
                @endif
            </div>
