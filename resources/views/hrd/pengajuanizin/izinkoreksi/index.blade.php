@extends('layouts.app')
@section('titlepage', 'Izin Koreksi')

@section('content')
@section('navigasi')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h4 class="mb-0">Izin Koreksi</h4>
            <small class="text-muted">Pengajuan Izin Koreksi Karyawan</small>
        </div>
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb mb-0" style="font-size: 13px">
                ...
                <li class="breadcrumb-item active">Izin Koreksi</li>
            </ol>
        </nav>
    </div>
@endsection
...
<div class="row">
    <div class="col-12">
        ...
        {{-- Filter Section --}}
        <form action="{{ route('izinkoreksi.index') }}">
        ...
        <div class="card shadow-sm border mt-2">
            <div class="card-header border-bottom py-3" style="background-color: #002e65; border-radius: 0.375rem 0.375rem 0 0;">
                <div class="d-flex justify-content-between align-items-center">
                    <h6 class="m-0 fw-bold text-white"><i class="ti ti-edit me-2"></i>Data Izin Koreksi</h6>
                    @can('izinkoreksi.create')
            ...
            <div class="table-responsive text-nowrap">
                <table class="table table-hover table-striped text-nowrap">
                    ...
                    <tbody>
                        @foreach ($izinkoreksi as $d)
                            <tr>
                                <td>{{ $d->kode_izin_koreksi }}</td>
                                ...
                                <td>
                                    <div class="d-flex justify-content-center gap-1">
                                        <a href="#" class="btnShow text-info"
                                            kode_izin_koreksi="{{ Crypt::encrypt($d->kode_izin_koreksi) }}" data-bs-toggle="tooltip"
                                            title="Detail">
                                        ...
                                        @can('izinkoreksi.edit')
                                            ...
                                            @endif
                                        @endcan
                                        @can('izinkoreksi.delete')
                                            @if (in_array($level_user, $level_hrd))
                                                @if ($d->status == 0)
                                                    <form class="delete-form"
                                                        action="{{ route('izinkoreksi.delete', Crypt::encrypt($d->kode_izin_koreksi)) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('DELETE')
                                                ...
                                                @else
                                                    @if (!empty($d->hrd) && empty($d->direktur))
                                                        <form method="POST" name="deleteform" class="deleteform"
                                                            action="{{ route('izinkoreksi.cancel', Crypt::encrypt($d->kode_izin_koreksi)) }}">
                                                            @csrf
                                                            @method('DELETE')
...
<script>
    $(function() {
        ...
        $("#btnCreate").click(function() {
            $("#modal").modal("show");
            loading();
            $("#modal").find(".modal-title").text("Buat Izin koreksi");
            $("#loadmodal").load("/izinkoreksi/create");
        });
        ...
        $(".btnEdit").click(function() {
            ...
            $("#modal").modal("show");
            loading();
            $("#modal").find(".modal-title").text("Edit Izin koreksi");
