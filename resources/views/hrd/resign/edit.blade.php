<form action="{{ route('resign.update', Crypt::encrypt($resign->kode_resign)) }}" method="POST" id="formResign">
    @csrf
    @method('PUT')
    <x-input-with-icon icon="ti ti-barcode" label="Auto" name="kode_resign" value="{{ $resign->kode_resign }}" disabled="true" hideLabel="true" />
    <x-input-with-icon icon="ti ti-calendar" label="Tanggal" name="tanggal" value="{{ $resign->tanggal }}" datepicker="flatpickr-date" hideLabel="true" />
    <x-select label="Karyawan" name="nik" :data="$karyawan" key="nik" textShow="nama_karyawan" select2="select2Nik" showKey="true"
        selected="{{ $resign->nik }}" hideLabel="true" />
    <x-select label="Kategori JMK" name="kode_kategori" :data="$kategori_jmk" key="kode_kategori" textShow="nama_kategori"
        selected="{{ $resign->kode_kategori }}" hideLabel="true" />
    <x-input-with-icon icon="ti ti-file-text" label="Keterangan" name="keterangan" value="{{ $resign->keterangan }}" hideLabel="true" />
