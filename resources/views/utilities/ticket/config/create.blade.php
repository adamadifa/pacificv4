@extends('layouts.app')
@section('titlepage', 'Konfigurasi Approval Tiket')

@section('content')
@section('navigasi')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h4 class="mb-0">Konfigurasi Approval Tiket</h4>
            <small class="text-muted">Rancang alur persetujuan tiket secara dinamis per departemen dan cabang.</small>
        </div>
...
<style>
    ...
    .step-number {
        ...
        font-weight: 700; font-size: 12px; margin-right: 15px; flex-shrink: 0;
    ...
    .role-item:hover { border-color: #4b6cb7; box-shadow: 0 8px 15px rgba(0, 0, 0, 0.05); transform: translateX(4px); }
    .drag-handle { color: #adb5bd; cursor: move; font-size: 1.2rem; padding: 1rem; }
    .ui-state-highlight {
        height: 80px; background: rgba(75, 108, 183, 0.03);
        border: 2px dashed #4b6cb7; border-radius: 14px;
        margin-bottom: 8px; list-style: none;
    }
    .empty-state {
        border: 2px dashed #dee2e6; border-radius: 16px;
        padding: 4rem 2rem; text-align: center; background: rgba(255, 255, 255, 0.5);
    }
    .btn-add-role { border-radius: 12px; padding: 0.6rem 1.5rem; font-weight: 600; transition: all 0.3s; }
    ...
    .gradient-btn {
        background: linear-gradient(135deg, #4b6cb7 0%, #182848 100%);
        border: none; box-shadow: 0 4px 15px rgba(75, 108, 183, 0.3);
        padding: 10px 24px; border-radius: 12px; font-weight: 600; transition: all 0.3s; color: white;
    }
    .gradient-btn:hover { transform: translateY(-2px); box-shadow: 0 8px 25px rgba(75, 108, 183, 0.4); background: linear-gradient(135deg, #5c7fcc 0%, #213661 100%); color: white; }
    .admin-terminal {
        background: #fff3cd; border: 1px solid #ffc107; border-radius: 10px;
        padding: 10px 16px; font-size: 12px; font-weight: 600; color: #856404;
        display: flex; align-items: center; gap: 8px;
    }
</style>

<form action="{{ route('ticketconfig.store') }}" method="POST" id="formConfig">
    @csrf
    <div class="row g-4">
            ...
            <div class="card config-card h-100">
                ...
                <div class="card-body pt-4">
                    <p class="text-muted small mb-4">
                        Pilih departemen dan cabang yang akan menggunakan alur persetujuan ini. Kosongkan untuk berlaku ke semua.
                    </p>
                    <div class="form-group mb-4">
                        ...
                        <small class="text-muted">Biarkan kosong agar berlaku untuk semua departemen.</small>
                    </div>
                    ...
                    <div class="form-group mb-4">
                        ...
                        <small class="text-muted">Biarkan kosong agar berlaku untuk semua cabang.</small>
                    </div>

                    <div class="bg-light rounded-3 p-3">
                        <p class="small mb-1 fw-semibold text-muted">Prioritas Konfigurasi</p>
                        <small class="text-muted">Sistem akan memilih konfigurasi yang paling spesifik (Dept + Cabang) terlebih dahulu, lalu Dept saja, lalu Cabang saja, lalu default global.</small>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card config-card h-100">
                ...
                <div class="card-body pt-4">
                    ...
                    <div class="admin-terminal mt-3" id="admin-terminal-indicator" style="display:none;">
        ...
        $(function() {
            ...
            function addRole(role) {
                ...
                $(".role-item").each(function() {
                    if ($(this).data('role') === role) exists = true;
                });
                if (exists) {
