            ...
            <div class="card h-100 border shadow-sm">
                ...
                <div class="card-body pt-3">
                    <div class="row g-3">
                        ...
                        <div class="col-sm-6">
                            <div class="p-3 border rounded" style="background-color: #fafbfc; border-left: 4px solid #0ea5e9 !important;">
                                <small class="text-muted d-block text-uppercase fw-bold mb-1" style="font-size: 11px; letter-spacing: 0.5px;">Tanggal</small>
                                <span class="fs-5 fw-bold text-dark">{{ DateToIndo($bpb->tanggal) }}</span>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="p-3 border rounded" style="background-color: #fafbfc; border-left: 4px solid #10b981 !important;">
                                <small class="text-muted d-block text-uppercase fw-bold mb-1" style="font-size: 11px; letter-spacing: 0.5px;">Departemen</small>
                                <span class="fs-5 fw-semibold text-dark">{{ $bpb->nama_dept }}</span>
                            </div>
                        </div>
                        <div class="col-sm-6">
                            <div class="p-3 border rounded" style="background-color: #fafbfc; border-left: 4px solid #f59e0b !important;">
                                <small class="text-muted d-block text-uppercase fw-bold mb-1" style="font-size: 11px; letter-spacing: 0.5px;">Cabang</small>
                        ...
                        <div class="col-12">
                            <div class="d-flex align-items-center gap-2 mt-1 px-1">
                                ...
                                <span class="text-muted small fw-semibold">User Input: <strong class="text-dark">{{ $bpb->nama_user }}</strong></span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-body d-flex flex-column justify-content-between pt-3">
                    <div>
                        ...
                        <div class="progress mb-4" style="height: 12px; border-radius: 6px;">
                            <div class="progress-bar bg-success progress-bar-striped progress-bar-animated" role="progressbar" style="width: {{ $percentComplete }}%" aria-valuenow="{{ $percentComplete }}" aria-valuemin="0" aria-valuemax="100"></div>
                        </div>
                    </div>
                    ...
                    <div class="row g-2 text-center mb-1">
                        ...
                        <div class="col-4">
                            <div class="p-2 border rounded" style="background-color: #fafbfc; border-top: 3px solid #10b981 !important;">
                                <h5 class="mb-0 fw-bold text-success">{{ formatAngkaDesimal($totalDiserahkan) }}</h5>
                                <small class="text-muted fw-semibold" style="font-size: 10px;">Diserahkan</small>
                            </div>
                        </div>
                        <div class="col-4">
                            <div class="p-2 border rounded" style="background-color: #fafbfc; border-top: 3px solid #ef4444 !important;">
            ...
            <table class="table table-hover table-bordered align-middle mb-0">
                <thead style="background-color: #002e65;">
                    <tr>
                        ...
                        @endif
                        <th class="text-white" style="width: 8%">Satuan</th>
                ...
                <tbody class="table-border-bottom-0">
                    ...
                    @foreach ($detail as $d)
                        ...
                        <tr>
                            ...
                            <td><span class="badge bg-secondary text-white font-monospace shadow-xs">{{ $d->kode_barang }}</span></td>
                            <td>
                                <span class="fw-semibold text-dark">{{ textCamelCase($d->nama_barang) }}</span>
                            </td>
                            ...
                            <td class="text-center">
                                <div class="d-flex align-items-center gap-2 justify-content-center">
                                    <div class="progress w-100 shadow-xs" style="height: 6px; min-width: 60px; border-radius: 3px;">
                                        <div class="progress-bar bg-success" role="progressbar" style="width: {{ $itemProgress }}%"></div>
                                    </div>
        ...
        <div class="card shadow-sm mb-4 border" style="border-left: 4px solid #10b981 !important;">
            <div class="card-body d-flex flex-column flex-sm-row justify-content-between align-items-sm-center gap-3 py-3">
                <div class="d-flex align-items-center gap-2">
                    <i class="ti ti-calendar-event fs-2 text-success"></i>
                    <div>
                        <h6 class="mb-0 text-dark fw-bold">Konfirmasi Penyerahan</h6>
                        <small class="text-muted">Tentukan tanggal serah terima dan simpan data penyerahan barang</small>
                    </div>
                </div>
                <div class="d-flex align-items-center gap-3 flex-wrap flex-sm-nowrap">
                    ...
                    <button type="submit" class="btn btn-success px-4 shadow-sm waves-effect waves-light d-flex align-items-center gap-2 h-100 align-self-end mb-1" style="height: 38px;">
...
<div class="card shadow-sm border mt-4">
    ...
    <div class="card-body pt-3">
        @forelse ($serahTerima as $s)
            @php
                $cardBorderLeft = $s->diterima == 1 ? 'border-left: 4px solid #10b981 !important;' : 'border-left: 4px solid #f59e0b !important;';
            @endphp
            <div class="card border mb-3 shadow-xs" style="{{ $cardBorderLeft }}">
                <div class="card-header py-2 px-3 d-flex flex-wrap justify-content-between align-items-center gap-2 border-bottom" style="background-color: #fafbfc;">
                    <div class="d-flex align-items-center gap-2">
                    ...
                    <div>
                        ...
                        @else
                            <span class="badge bg-success text-white shadow-sm px-3 py-1_5"><i class="ti ti-circle-check me-1"></i>Sudah Diterima</span>
                        @endif
                    </div>
                </div>
                        ...
                        <table class="table table-hover table-bordered align-middle mb-0">
                            ...
                            <tbody>
                                @foreach ($historyDetail[$s->no_bukti] ?? [] as $h)
                                    <tr>
                                        <td class="text-center fw-semibold">{{ $loop->iteration }}</td>
                                        <td><span class="badge bg-secondary text-white font-monospace shadow-xs">{{ $h->kode_barang }}</span></td>
                                        <td class="fw-semibold text-dark">{{ $h->nama_barang }}</td>
                                        <td class="text-end" style="max-width: 100px;">
                                            <input type="text" class="form-control form-control-sm text-end editJumlah px-2 shadow-none"
                                                value="{{ $h->jumlah }}" data-no_bukti="{{ $h->no_bukti }}"
                                                data-kode_barang="{{ $h->kode_barang }}"
                                                style="border-color: #cbd5e1;"
                                                {{ $s->diterima != 0 || $totalSisa <= 0 ? 'readonly' : '' }}>
                                        </td>
                                        <td><span class="badge bg-info text-white shadow-xs">{{ $h->satuan }}</span></td>
                                        <td>
                                            <input type="text" class="form-control form-control-sm editKeterangan shadow-none"
                                                value="{{ $h->keterangan }}" data-no_bukti="{{ $h->no_bukti }}"
                                                data-kode_barang="{{ $h->kode_barang }}" placeholder="Edit keterangan..."
                                                style="border-color: #cbd5e1;"
                                                {{ $s->diterima != 0 ? 'readonly' : '' }}>
                                        </td>
                                        @if ($s->diterima == 0)
                                            <td class="text-center">
                                                <a href="#" class="deleteHistoryBtn text-danger d-inline-block p-1"
        ...
        @empty
            <div class="text-center py-5 text-muted border border-dashed rounded" style="background-color: #fafbfc; border-color: #cbd5e1 !important;">
                <i class="ti ti-history-off fs-1 d-block mb-2 text-secondary"></i>
                <span class="fw-semibold">Belum ada history serah terima untuk BPB ini.</span>
            </div>
        @endforelse
    </div>
</div>
