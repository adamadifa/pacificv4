<form action="{{ route('barangmasukgudanglogistik.update',Crypt::encrypt($barangmasuk->no_bukti)) }}" method="post" id="formeditbarangmasukgudanglogistik">
    @csrf
    @method('PUT')
    <x-input-with-icon icon="ti ti-barcode" label="No. Bukti Pemasukan" name="no_bukti" value="{{ $barangmasuk->no_bukti }}" hideLabel="true" />
    <x-input-with-icon icon="ti ti-calendar" label="Tanggal" name="tanggal" datepicker="flatpickr-date" value="{{ $barangmasuk->tanggal }}" hideLabel="true" />
