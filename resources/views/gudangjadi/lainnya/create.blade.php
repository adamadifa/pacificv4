<form action="{{ route('lainnyagudangjadi.store') }}" method="post" id="formcreateLainnya">
    @csrf
    ...
    <div class="form-check form-check-inline mt-3">
        <input class="form-check-input" type="radio" name="in_out" id="inlineRadio1" value="I">
        <label class="form-check-label" for="inlineRadio1">IN</label>
    </div>
    <div class="form-check form-check-inline">
        <input class="form-check-input" type="radio" name="in_out" id="inlineRadio2" value="O">
...
<script>
    $(function() {
        const formCreate = $("#formcreateLainnya");
        ...
        formCreate.submit(function() {
            ...
            if (formCreate.find('#loaddetail tr').length == 0) {
                ...
                Swal.fire({
                    ...
                    showConfirmButton: true,
                    didClose: (e) => {
                        formCreate.find("#keterangan").focus();
                    },
                });

                return false;
            } else if ($('input[type=radio][name=in_out]:checked').length == 0) {
                Swal.fire({
                    title: "Oops!",
