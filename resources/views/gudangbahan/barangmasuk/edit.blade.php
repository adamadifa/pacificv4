<form action="{{ route('barangmasukgudangbahan.update', Crypt::encrypt($barangmasuk->no_bukti)) }}" method="post"
   id="formcreateBarangmasukgudangbahan">
   @csrf
   @method('PUT')
   <x-input-with-icon icon="ti ti-barcode" label="No. Bukti Pemasukan" name="no_bukti"
      value="{{ $barangmasuk->no_bukti }}" />
      ...
      <select name="kode_asal_barang" id="kode_asal_barang" class="form-select">
         ...
         @foreach ($list_asal_barang as $d)
            <option value="{{ $d['kode_asal_barang'] }}"
               {{ $barangmasuk->kode_asal_barang == $d['kode_asal_barang'] ? 'selected' : '' }}>
               ...
               @foreach ($detail as $d)
                  ...
                  <tr id="index_{{ $index }}">
                     ...
                     <td class="text-center">
                        <a href="#" kode_barang="{{ $index }}" class="delete"><i
