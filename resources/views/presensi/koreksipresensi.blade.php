@if (!empty($presensi))
    ...
    @else
    <form action="{{ route('presensi.storekoreksipresensi') }}" method="POST" id="formKoreksiPresensi">
@endif
@csrf
<div class="row">
    <div class="col">
    ...
    <div class="col">
        ...
        <div class="form-group mb-3">
            <select name="kode_jadwal" id="kode_jadwal" class="form-select">
                ...
                @foreach ($jadwal as $d)
                    <option {{ !empty($presensi) && $presensi->kode_jadwal == $d->kode_jadwal ? 'selected' : '' }} value="{{ $d->kode_jadwal }}">{{ $d->nama_jadwal }}
                    </option>
                @endforeach
            </select>
    ...
    function loadjamkerja() {
        ...
        $.ajax({
            ...
            data: {
                ...
                'kode_jam_kerja': "{{ !empty($presensi) ? $presensi->kode_jam_kerja : '' }}"
            },
