<form action="#" id="formEditPotongan">
    ...
    <div class="form-group mb-3">
        <select name="kode_akun_potongan" id="kode_akun_potongan" class="form-select select2Kodeakunpotongan">
            ...
            @foreach ($coa as $d)
                <option value="{{ $d->kode_akun }}" {{ $datapotongan['kode_akun'] == $d->kode_akun ? 'selected' : '' }}>{{ $d->kode_akun }} -
                    {{ $d->nama_akun }}</option>
            @endforeach
        </select>
    </div>
    <div class="form-group mb-3">
        <button class="btn btn-primary w-100" id="btnPotongan"><i class="ti ti-send me-1"></i>Update</button>
    </div>
</form>
<script>
    $(function() {
