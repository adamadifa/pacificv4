<form action="{{ route('jurnalkoreksi.update', Crypt::encrypt($jurnalkoreksi->kode_jurnalkoreksi)) }}" id="formEditJurnalkoreksi"
    method="POST">
    @csrf
    @method('PUT')
    ...
    <div class="form-group mb-3">
        <select name="no_bukti" id="no_bukti" class="form-select select2Nobukti">
            ...
            <option value="{{ $jurnalkoreksi->no_bukti }}" selected>{{ $jurnalkoreksi->no_bukti }}</option>
        </select>
    </div>
    <div class="form-group mb-3">
        <select name="kode_barang" id="kode_barang" class="form-select select2Kodebarang">
            <option value="">Pilih Barang</option>
            <option value="{{ $jurnalkoreksi->kode_barang }}" selected>{{ $jurnalkoreksi->nama_barang }}</option>
        </select>
    </div>
    ...
    <div class="form-group mb-3">
        <select name="kode_akun_debet" id="kode_akun_debet" class="form-select select2Kodeakundebet">
            ...
            @foreach ($coa as $d)
                <option value="{{ $d->kode_akun }}" {{ $kode_akun_debet == $d->kode_akun ? 'selected' : '' }}>
    ...
    <div class="form-group mb-3">
        <select name="kode_akun_kredit" id="kode_akun_kredit" class="form-select select2Kodeakunkredit">
            ...
            @foreach ($coa as $d)
                <option value="{{ $d->kode_akun }}" {{ $kode_akun_kredit == $d->kode_akun ? 'selected' : '' }}>
                    {{ $d->kode_akun }} - {{ $d->nama_akun }}</option>
            @endforeach
        </select>
    </div>
...
<script>
    $(function() {
        const form = $("#formEditJurnalkoreksi");
