<div class="space-y-4">
    ...
    @forelse ($details as $d)
        ...
        <div class="bg-white rounded-2xl p-4 shadow-sm border border-slate-50">
            <div class="flex justify-between items-center mb-1">
                <span class="text-xs font-bold text-slate-800">{{ $d->nama_bank }}</span>
                <span class="text-sm font-black text-blue-600">Rp {{ formatAngka($d->jumlah) }}</span>
            </div>
            <p class="text-[10px] text-slate-400 font-medium">{{ $d->no_rekening }}</p>
        </div>
    @empty
    ...
    <div class="mt-8 p-6 bg-blue-600 rounded-[32px] shadow-lg shadow-blue-200 text-white">
        <p class="text-[10px] font-bold uppercase tracking-[2px] opacity-70 mb-1">Total Saldo</p>
