<form action="{{ route('pembayarantransfer.approvestore', Crypt::encrypt($transfer->kode_transfer)) }}" method="POST" id="formApprovetransfer">
   @csrf
   <div class="row">
      <div class="col">
         <table class="table">
      ...
      <div class="col">
         ...
         <div class="form-group mb-3" id="bank">
            <select name="kode_bank" id="kode_bank" class="form-select select2Kodebank">
               ...
               @foreach ($bank as $d)
                  <option value="{{ $d->kode_bank }}">{{ !empty($d->nama_bank_alias) ? $d->nama_bank_alias : $d->nama_bank }} {{ !empty($d->no_rekening) ? '(' . $d->no_rekening . ')' : '' }}
                  </option>
               @endforeach
            </select>
         </div>
