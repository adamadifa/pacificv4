            ...
            @hasanyrole($roles_show_cabang)
               <div class="col-lg-6 col-md-12 col-sm-12">
                  <x-select label="Pilih Cabang" name="kode_cabang" :data="$cabang" key="kode_cabang"
                     textShow="nama_cabang" select2="select2Kodecabang" showKey="true" upperCase="true" selected="{{ $targetkomisi->kode_cabang }}" />
               </div>
                  ...
                  <div class="form-group mb-3">
                     <select name="bulan" id="bulan" class="form-select">
                        ...
                        @foreach ($list_bulan as $d)
                           <option {{ $targetkomisi->bulan == $d['kode_bulan'] ? 'selected' : '' }} value="{{ $d['kode_bulan'] }}">{{ $d['nama_bulan'] }}</option>
                        @endforeach
                     </select>
                  </div>
               </div>
