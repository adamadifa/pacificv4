...
<head>
    ...
    <title>Rekap Retur Netto per Regional Multi Tahun {{ date('Y-m-d H:i:s') }}</title>
...
<body>
    <div class="header">
        <h4 class="title">
            REKAP RETUR NETTO PER REGIONAL MULTI TAHUN <br>
        </h4>
                ...
                @foreach ($regional_list as $reg)
                    <tr>
                        ...
                        @foreach ($years as $year)
                            ...
                            @for ($m = 1; $m <= 12; $m++)
                                @php
                                    $retur = $retur_map[$reg->kode_regional][$year][$m] ?? 0;
                                    $grand_totals[$year][$m] += $retur;
                                    $total_regional_year += $retur;
                                @endphp
                                <td align="right">{{ $retur != 0 ? formatAngka($retur) : '' }}</td>
                            @endfor
                            @php
