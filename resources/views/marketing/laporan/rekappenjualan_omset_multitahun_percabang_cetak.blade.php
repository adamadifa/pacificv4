...
<head>
    ...
    <title>Rekap Penjualan Omset per Cabang Multi Tahun {{ date('Y-m-d H:i:s') }}</title>
...
<body>
    <div class="header">
        <h4 class="title">
            REKAP PENJUALAN OMSET PER CABANG MULTI TAHUN <br>
        </h4>
                ...
                @foreach ($cabang_list as $cb)
                    <tr>
                        ...
                        @foreach ($years as $year)
                            ...
                            @for ($m = 1; $m <= 12; $m++)
                                @php
                                    $netto = $netto_map[$cb->kode_cabang][$year][$m] ?? 0;
                                    $grand_totals[$year][$m] += $netto;
                                    $total_cabang_year += $netto;
                                @endphp
