...
<head>
    ...
    <title>Rekap Piutang per Regional Multi Tahun {{ date('Y-m-d H:i:s') }}</title>
...
<body>
    <div class="header">
        <h4 class="title">
            REKAP PIUTANG PER REGIONAL MULTI TAHUN <br>
        </h4>
                ...
                @foreach ($regional_list as $reg)
                    <tr>
                        ...
                        @foreach ($years as $year)
                            ...
                            @for ($m = 1; $m <= 12; $m++)
                                @php
                                    $piutang = $piutang_map[$reg->kode_regional][$year][$m] ?? 0;
                                    $grand_totals[$year][$m] += $piutang;
                                    $total_regional_year += $piutang;
                                @endphp
