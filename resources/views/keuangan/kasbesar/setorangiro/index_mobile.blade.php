...
@section('navigasi')
    <div class="d-flex justify-content-between align-items-center mb-1">
        <div>
            <h4 class="mb-0 fw-bold text-slate-800">Setoran Giro</h4>
            <small class="text-muted" style="font-size: 11px;">Manajemen setoran dari giro pelanggan (Mobile View)</small>
        </div>
    </div>
@endsection
...
<div class="row">
    <div class="col-12">
                ...
                <div class="flex-grow-1 ps-1">
                    ...
                    <ul class="list-unstyled mb-0 text-slate-600 d-flex flex-column gap-1.5" style="font-size: 11.5px; line-height: 1.4;">
                        <li class="d-flex align-items-center gap-1">
                            ...
                            <span>Tap <span class="badge bg-label-success px-1.5 py-0.5" style="font-size: 10px; border-radius: 4px;"><i class="ti ti-external-link me-0.5" style="font-size: 11px;"></i>Setorkan</span> untuk input setoran giro.</span>
                        </li>
                        <li class="d-flex align-items-center gap-1">
                            <i class="ti ti-circle-chevron-right text-primary" style="font-size: 10px;"></i>
                            <span>Tap <span class="badge bg-label-danger px-1.5 py-0.5" style="font-size: 10px; border-radius: 4px;"><i class="ti ti-square-rounded-x me-0.5" style="font-size: 11px;"></i>Batalkan</span> untuk hapus setoran giro.</span>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <div class="card border mb-3 shadow-sm" style="border-radius: 12px; overflow: hidden;">
            ...
            <div class="collapse {{ Request('dari') || Request('sampai') || Request('kode_cabang_search') || Request('no_giro') || Request('nama_pelanggan_search') || Request('kode_salesman_search') || Request('status') !== null ? 'show' : '' }}" id="collapseFilter">
                <div class="card-body p-3 border-top bg-white">
                    <form action="{{ route('setorangiro.index') }}">
                        <div class="row g-2">
                            ...
                            @endrole
                            <div class="col-12">
                                <x-input-with-icon label="No. Giro" icon="ti ti-barcode" name="no_giro" value="{{ Request('no_giro') }}" hideLabel="true" placeholder="No. Giro" />
                            </div>
                            <div class="col-12">
        ...
        @if ($giro->isEmpty())
            ...
            <div class="card border shadow-sm rounded-3 py-5 px-3 text-center my-4">
                ...
                <h5 class="fw-bold text-slate-800" style="font-size: 15px;">Tidak Ada Data Setoran Giro</h5>
                <p class="text-muted mb-0" style="font-size: 12px;">Silakan sesuaikan filter tanggal atau pencarian Anda.</p>
            </div>
        @else
            <div class="giro-card-list">
                @foreach ($giro as $d)
                    <div class="card setoran-card mb-3 overflow-hidden">
                        ...
                        <div class="card-header py-2 px-3 border-bottom d-flex justify-content-between align-items-center" style="background-color: #fcfdfe;">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                <span class="text-slate-800 fw-bold" style="font-size: 12px; letter-spacing: 0.2px;">{{ $d->no_giro }}</span>
                            </div>
                            <div>
                                @if ($d->status == '1')
                        ...
                        <div class="card-body p-3">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3 bg-light p-2 rounded-3">
                                <span class="text-muted" style="font-size: 11px;">Jumlah Giro</span>
                                <span class="fw-bold text-primary" style="font-size: 15px;">Rp {{ formatAngka($d->total) }}</span>
                            </div>

                            {{-- Collapsible Breakdown Details --}}
                            <div class="collapse pt-3 mt-2 border-top" id="detail-giro-{{ $d->no_giro }}">
                            ...
                            <div class="d-flex justify-content-between align-items-center mt-3 pt-2 border-top">
                                <button class="btn btn-sm btn-outline-secondary py-1 px-2 d-inline-flex align-items-center gap-1 collapsed" 
                                        type="button" 
                                        data-bs-toggle="collapse" 
                                        data-bs-target="#detail-giro-{{ $d->no_giro }}" 
                                        aria-expanded="false" 
                                        aria-controls="detail-giro-{{ $d->no_giro }}"
                                    ...
                                    @can('setorangiro.create')
                                        @if (empty($d->tanggal_disetorkan))
                                            <a href="#" class="btn btn-sm btn-success py-1 px-2 btnCreate d-inline-flex align-items-center gap-1"
                                               kode_giro="{{ Crypt::encrypt($d->kode_giro) }}" style="font-size: 11px; border-radius: 6px;">
            ...
            <div class="mt-3 mb-4 d-flex justify-content-center">
                {{ $giro->links('vendor.pagination.bootstrap-5') ?? $giro->links() }}
            </div>
        @endif
    </div>
</div>
