<form action="{{ route('setoranpenjualan.update', Crypt::encrypt($setoranpenjualan->kode_setoran)) }}" method="POST" id="formUpdatesetoran">
    @csrf
    @method('PUT')
...
<script>
    $(function() {
        ...
        function getsalesmanbyCabang() {
            ...
            const kode_salesman = "{{ $setoranpenjualan->kode_salesman }}";
            //alert(selected);
            $.ajax({
                type: 'POST',
