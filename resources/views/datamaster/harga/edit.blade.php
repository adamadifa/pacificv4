<form action="{{ route('harga.update', Crypt::encrypt($harga->kode_harga)) }}" id="formeditHarga" method="POST">
    ...
    @method('PUT')
    {{-- {{ $harga->kode_produk }} --}}
    <x-input-with-icon icon="ti ti-barcode" label="Kode Harga" name="kode_harga" value="{{ $harga->kode_harga }}"
        disabled="true" />
    <x-select label="Produk" name="kode_produk" :data="$produk" key="kode_produk" textShow="nama_produk"
        selected="{{ $harga->kode_produk }}" upperCase="true"
    ...
    <x-select label="Kategori" name="kode_kategori_salesman" :data="$kategori_salesman" key="kode_kategori_salesman"
        textShow="nama_kategori_salesman" selected="{{ $harga->kode_kategori_salesman }}" upperCase="true"
        disabled="{{ !auth()->user()->hasRole('super admin') ? 'disabled' : '' }}" />
    @hasanyrole('super admin')
    <x-select label="Cabang" name="kode_cabang" :data="$cabang" key="kode_cabang" textShow="nama_cabang"
        selected="{{ $harga->kode_cabang }}" select2="select2Kodecabang" />
    @endhasanyrole
    @hasanyrole('super admin')
    <div class="form-group mb-3">
...
<script src="{{ asset('assets/js/pages/harga/edit.js') }}?v=1.0.2"></script>
<script>
    $(".money").maskMoney();
</script>
<script>
