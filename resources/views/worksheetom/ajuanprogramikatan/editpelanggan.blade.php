                ...
                @while (strtotime($current_date) <= strtotime($end_date))
                    <tr class="targetbulanan">
                        ...
                        <td>
                            ...
                            @endphp
                            {{-- <input type="text" name="target_perbulan[]"
                                value="{{ isset($array_target[$keytarget]) ? $array_target[$keytarget] : '' }}" style="text-align: right"
                                class="noborder-form number-separator"> --}}
                            {{ isset($array_target[$keytarget]) ? $array_target[$keytarget] : '' }}
                        </td>
                    </tr>
                    @php
                        $current_date = date('Y-m-d', strtotime('+1 month', strtotime($current_date)));
        ...
        <div class="row">
            ...
            <div class="col">
                <x-input-with-icon-label label="Budget RSM" name="budget_rsm" icon="ti ti-file-description" placeholder="Budget RSM"
                    align="right" value="{{ formatAngka($detail->budget_rsm) }}" readonly />
            </div>
            <div class="col">
                <x-input-with-icon-label label="Budget GM" name="budget_gm" icon="ti ti-file-description" placeholder="Budget GM" align="right"
...
<script>
    $(document).ready(function() {
        ...
        function calculateTargetPerBulan() {
            ...
            if (sisa > 0) {
                $('input[name="target_perbulan[]"]:last').val(10);
            }
