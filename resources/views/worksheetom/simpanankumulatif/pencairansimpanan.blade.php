@extends('layouts.app')
@section('titlepage', 'Pencairan Simpanan Kumulatif')

@section('content')
@section('navigasi')
    <span>Pencairan Simpanan Kumulatif</span>
@endsection
<div class="row">
                                    ...
                                    <tbody>
                                        @foreach ($pencairan as $d)
                                            <tr>
                                                ...
                                                <td>
                                                    {{ $d->metode_pembayaran == 'TF' ? 'Transfer' : ($d->metode_pembayaran == 'TN' ? 'Tunai' : $d->metode_pembayaran) }}
                                                </td>
                                                ...
                                                <td>
                                                    <div class="d-flex">
                                                        @if (
                                                            $d->status == 0 &&
                                                                auth()->user()->hasRole(['kepala admin', 'super admin', 'operation manager', 'sales marketing manager']))
                                                            <form method="POST" name="deleteform" class="deleteform"
                                                                action="{{ route('pencairanprogram.deletepencairansimpanan', Crypt::encrypt($d->kode_pencairan)) }}">
                                                                @csrf
                                                                @method('DELETE')
                                                        ...
                                                        @if (auth()->user()->hasRole(['manager keuangan', 'staff keuangan', 'super admin']))
                                                            <a href="{{ route('pencairanprogram.cetakpencairansimpanan', Crypt::encrypt($d->kode_pencairan)) }}"
...
<script>
    $(".btnApprove").click(function(e) {
        ...
        $("#loadmodalapprove").load('/pencairanprogram/' + kode_pencairan + '/approvepencairansimpanan');
    });
</script>
@endpush
