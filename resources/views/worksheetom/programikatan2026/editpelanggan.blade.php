<form action="{{ route('programikatan2026.updatepelanggan', [Crypt::encrypt($detail->no_pengajuan), Crypt::encrypt($detail->kode_pelanggan)]) }}"
    ...
    <x-input-with-icon-label label="Qty Rata - rata Penjualan Tahun Sebelumnya" name="qty_avg" icon="ti ti-file-description"
        placeholder="Qty Rata - rata Penjualan Tahun Sebelumnya" align="right" readonly value="{{ formatAngka($detail->qty_avg) }}" disabled />

    <x-input-with-icon-label label="Total Target Tambahan" name="target" icon="ti ti-file-description" placeholder="Total Target Tambahan" align="right"
        value="{{ formatAngka($total_target_tambahan) }}" />
