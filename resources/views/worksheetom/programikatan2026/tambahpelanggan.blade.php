<form action="{{ route('programikatan2026.storepelanggan', Crypt::encrypt($ajuanprogramikatan->no_pengajuan)) }}"
    ...
    <x-input-with-icon label="Qty Rata - rata Penjualan Tahun Sebelumnya" name="qty_avg" icon="ti ti-file-description"
        placeholder="Qty Rata - rata Penjualan Tahun Sebelumnya" align="right" readonly />
    <div class="row">
        <div class="col" id="gethistoripelangganprogram"></div>
    </div>
    <x-input-with-icon label="Total Target Tambahan" name="target" icon="ti ti-file-description" placeholder="Total Target Tambahan"
        align="right" />
        ...
        <tbody>
            ...
            @while (strtotime($current_date) <= strtotime($end_date))
                <tr class="targetbulanan">
                    ...
                    <td>
                        <input type="text" name="avg_perbulan[]" class="noborder-form avg-input" readonly style="text-align: right">
                    </td>
                    <td>
                    ...
                    <td>
                        <input type="text" name="total_target[]" class="noborder-form total-input" readonly style="text-align: right">
                    </td>
                </tr>
                @php
    ...
    $(document).ready(function() {
        ...
        function calculateTotalTarget() {
            ...
            $('input[name="target_perbulan[]"]').each(function() {
                let value = $(this).val().replace(/\./g, '');
                if (!isNaN(value) && value.length != 0) {
                    totalpenambah += parseFloat(value);
                }
            });
        ...
        $('#target').on('keyup keydown change', function() {
            ...
            // calculateTotalTarget(); // No need to verify against itself immediately? 
            // Actually target per bulan updates inputs, which triggers nothing?
            // target per bulan updates val() of inputs using .val(). This does NOT trigger keyup/change events automatically.
            // So we need to calculate row totals and THEN calculate grand total.
            
            // We need to iterate all rows and update row total first
            $('tr.targetbulanan').each(function() {
        ...
        function calculateRowTotal(row) {
            let avg = row.find('.avg-input').val().replace(/\./g, '');
            let target = row.find('input[name="target_perbulan[]"]').val().replace(/\./g, '');

            avg = avg == "" ? 0 : parseInt(avg);
            target = target == "" ? 0 : parseInt(target);
            let total = avg + target;
            row.find('.total-input').val(convertToRupiah(total));
        }
