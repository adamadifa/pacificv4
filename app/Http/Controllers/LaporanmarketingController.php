...
class LaporanmarketingController extends Controller
{
    ...
    public function cetakpenjualanformatstandar(Request $request)
    {
        ...
        if (!empty($request->status_sampel)) {
            if ($request->status_sampel == '1') {
                $qpenjualan->where('marketing_penjualan.status_sampel', 1);
            } else if ($request->status_sampel == '2') {
    ...
    public function cetakrekappenjualanallcabang(Request $request)
    {
        ...
        $status_sampel_query = " AND marketing_penjualan.status_sampel = 0";
        if (!empty($request->status_sampel)) {
            if ($request->status_sampel == '1') {
                $status_sampel_query = " AND marketing_penjualan.status_sampel = 1";
            } else if ($request->status_sampel == '2') {
                $status_sampel_query = " AND marketing_penjualan.status_sampel = 0";
            }
        }
    ...
    public function cetakkasbesardetail(Request $request)
    {
        ...
        $query->where('marketing_penjualan.status_sampel', 0);
        ...
        $qvoucher->where('marketing_penjualan.status_sampel', 0);
    ...
    public function cetakkasbesarlhp(Request $request)
    {
        ...
        $querygiro->where('marketing_penjualan.status_sampel', 0);
        ...
        $querytransfer->where('marketing_penjualan.status_sampel', 0);
        ...
        $queryvoucher->where('marketing_penjualan.status_sampel', 0);
    ...
    public function cetakretur(Request $request)
    {
        ...
        $qretur->where('marketing_penjualan.status_sampel', 0);
        if (!empty($kode_cabang)) {
    ...
    public function cetaktunaikredit(Request $request)
    {
        ...
        $queryPotongan->where('marketing_penjualan.status_sampel', 0);
    ...
    public function cetakomsetpelanggan(Request $request)
    {
        ...
        if ($request->format_laporan == 1) {
            ...
            $qdetailpenjualan->where('marketing_penjualan.status_sampel', 0);
            if (!empty($kode_cabang)) {
    ...
    public function cetakanalisatransaksi(Request $request)
    {
        ...
        $querypenjualan->where('marketing_penjualan.status_sampel', 0);
        if (!empty($kode_cabang)) {
        ...
        $querypembayaran->where('marketing_penjualan.status_sampel', 0);
        if (!empty($kode_cabang)) {
        ...
        $queryqty->where('marketing_penjualan.status_sampel', 0);
        if (!empty($kode_cabang)) {
    ...
    public function cetakkartupiutang(Request $request)
    {
        ...
        $querysaldoawalbulanini->where('marketing_penjualan.status_sampel', 0);
    ...
    public function cetaklebihsatufaktur(Request $request)
    {
        ...
        $qpelangganBelumlunas2faktur->where('marketing_penjualan.status_sampel', 0);
    ...
    public function cetaklhp(Request $request)
    {
        ...
        $qhistoribayar->where('marketing_penjualan.status_sampel', 0);
    ...
    public function cetakkomisisalesman(Request $request)
    {
        ...
        if ($request->format_komisi == '2') {
            // Target dihitung dari rata-rata realisasi Januari - Juni tahun berjalan
            $jan_juni_dari = $request->tahun . "-01-01";
            $jan_juni_sampai = $request->tahun . "-06-30";

            $selectColumntarget_avg = [];
            foreach ($kategori_komisi as $k) {
                // Rata-rata target = Total realisasi qty (jumlah / isi_pcs_dus) dari Jan-Jun dibagi 6 bulan
                $selectColumntarget_avg[] = DB::raw("SUM(IF(produk.kode_kategori_komisi='$k->kode_kategori', jumlah / isi_pcs_dus, 0)) / 6 as `target_$k->kode_kategori`");
            }

            // Salesman yang memiliki target adalah salesman yang aktif di cabang tersebut
            $salesman_target = Salesman::select('kode_salesman')
                ->where('kode_cabang', $kode_cabang)
                ->where('status_aktif_salesman', 1);

            $subqueryTarget = Detailpenjualan::join('marketing_penjualan', 'marketing_penjualan_detail.no_faktur', '=', 'marketing_penjualan.no_faktur')
                ->join('produk_harga', 'marketing_penjualan_detail.kode_harga', '=', 'produk_harga.kode_harga')
                ->join('produk', 'produk_harga.kode_produk', '=', 'produk.kode_produk')
                ->join('salesman', 'marketing_penjualan.kode_salesman', '=', 'salesman.kode_salesman')
                ->where('salesman.kode_cabang', $kode_cabang)
                ->whereBetween('marketing_penjualan.tanggal_pelunasan', [$jan_juni_dari, $jan_juni_sampai])
                ->where('status_promosi', 0)
                ->where('status_batal', 0)
                ->where('marketing_penjualan.status_sampel', 0)
                ->select('marketing_penjualan.kode_salesman', ...$selectColumntarget_avg)
        ...
        $query_program_ikatan = DB::table('mkt_ikatan_detail_2026')
            ...
            ->whereMonth('mkt_ikatan_2026.tanggal', '<=', $request->bulan)
        ...
        $data['komisi'] = Salesman::select(
            ...
            ->leftJoin(
                DB::raw("(
                    ...
                    `marketing_penjualan`
                    INNER JOIN `salesman` ON `marketing_penjualan`.`kode_salesman` = `salesman`.`kode_salesman`
                    WHERE `tanggal` BETWEEN '$dari' AND '$sampai' AND `status_batal` = '0' AND marketing_penjualan.status_sampel = 0 AND salesman.kode_cabang = '$kode_cabang'
                    GROUP BY
                        marketing_penjualan.kode_salesman
                ) kunjungan"),
                function ($join) {
    ...
    public function cetakrekappenjualan(Request $request)
    {
        ...
        } else if ($request->jenis_laporan == 7) {
    ...
    public function cetakrekappenjualanmultitahun_percabang($kode_cabang, Request $request)
    {
        $roles_access_all_cabang = config('global.roles_access_all_cabang');
        $user = User::findorfail(auth()->user()->id);
        ...
        // Get Qty data grouped by cabang, product, year, and month
        $q_qty = DB::table('marketing_penjualan')
            ->select(
                'salesman.kode_cabang',
                'ph.kode_produk',
        ...
        if (!empty($kode_cabang)) {
            $q_qty->where('salesman.kode_cabang', $kode_cabang);
        }

        $qty_data = $q_qty
            ->groupBy('salesman.kode_cabang', 'ph.kode_produk', DB::raw('YEAR(marketing_penjualan.tanggal)'), DB::raw('MONTH(marketing_penjualan.tanggal)'))
            ->get();

        $qty_map = [];
        $active_product_ids = [];
        foreach ($qty_data as $q) {
            $qty_map[$q->kode_cabang][$q->kode_produk][$q->tahun][$q->bulan] = $q->qty;
            if ($q->qty > 0) {
                $active_product_ids[] = $q->kode_produk;
            }
        }
    ...
    public function cetakrekappenjualanmultitahun_perregional($kode_cabang, Request $request)
    {
        $roles_access_all_cabang = config('global.roles_access_all_cabang');
        $user = User::findorfail(auth()->user()->id);
        ...
        // Get Qty data grouped by regional, product, year, and month
        $q_qty = DB::table('marketing_penjualan')
            ->select(
        ...
        $qty_data = $q_qty
            ->groupBy('cabang.kode_regional', 'ph.kode_produk', DB::raw('YEAR(marketing_penjualan.tanggal)'), DB::raw('MONTH(marketing_penjualan.tanggal)'))
            ->get();

        $qty_map = [];
        $active_product_ids = [];
        foreach ($qty_data as $q) {
            $qty_map[$q->kode_regional][$q->kode_produk][$q->tahun][$q->bulan] = $q->qty;
            if ($q->qty > 0) {
                $active_product_ids[] = $q->kode_produk;
            }
        }
    ...
    public function cetakrekappenjualanproduk($kode_cabang, $request)
    {
        ...
        $queryretur->whereBetween('marketing_retur.tanggal', [$request->dari, $request->sampai]);
        $queryretur->where('marketing_penjualan.status_sampel', 0);
        if (!empty($request->kode_cabang)) {
            $queryretur->where('salesman.kode_cabang', $kode_cabang);
        }
        if (!empty($request->kode_salesman)) {
    ...
    public function cetakrekapretur($kode_cabang, Request $request)
    {
        ...
        $qproduk->where('marketing_penjualan.status_sampel', 0);
        if (!empty($request->kode_cabang)) {
    ...
    public function cetak_rekappenjualan($kode_cabang, Request $request)
    {
        ...
        $querydetail->where('marketing_penjualan.status_sampel', 0);
        ...
        $queryhistoribayar->join('marketing_penjualan', 'marketing_penjualan_historibayar.no_faktur', '=', 'marketing_penjualan.no_faktur');
        $queryhistoribayar->where('marketing_penjualan.status_sampel', 0);
    ...
    {
        if ($request->format == 'perjuli2026') {
            return $this->cetakinsentifomperjuli2026($request);
        }
        ...
        $saldoawal = Saldoawalpiutangpelanggan::where('bulan', $request->bulan)
            ->where('tahun', $request->tahun)->first();
        $saldoawal_date = $saldoawal ? $saldoawal->tanggal : $dari;
        $saldoawal_enddate = date('Y-m-t', strtotime($saldoawal_date));
        ...
        // $querysaldoawal->where('bulan', $bulan);
        // $querysaldoawal->where('tahun', $tahun);
        $querysaldoawal->where('marketing_saldoawal_piutang.kode_saldo_awal', $saldoawal ? $saldoawal->kode_saldo_awal : '');
    ...
    public function cetakinsentifomperjuli2026(Request $request)
    {
        $roles_access_all_cabang = config('global.roles_access_all_cabang');
        $user = User::findorfail(auth()->user()->id);
        ...
        $queryvisit = Visitpelanggan::query()
            ...
            ->select(
                'kode_cabang_baru as kode_cabang',
                DB::raw('COUNT(DISTINCT marketing_penjualan.kode_pelanggan) as total_validasi_kunjungan')
            )
        ...
        $queryopname = Dokumenopname::query()
            ->select(
                'kode_cabang',
                DB::raw('COUNT(DISTINCT tanggal) as total_opname_harian')
            )
        ...
        $query->leftjoinSub($queryvisit, 'validasikunjungan', function ($join) {
            $join->on('cabang.kode_cabang', '=', 'validasikunjungan.kode_cabang');
        });

        $query->leftjoinSub($queryopname, 'opnameharian', function ($join) {
            $join->on('cabang.kode_cabang', '=', 'opnameharian.kode_cabang');
        });
        ...
        return view('marketing.laporan.insentifom_perjuli2026_cetak', $data);
    }
