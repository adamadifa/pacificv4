...
class DashboardController extends Controller
{
    ...
    public function getdetailsaldoawal(Request $request)
    {
        ...
        $agent = new Agent();
        if ($agent->isMobile()) {
            return view('dashboard.mobile.getdetailsaldoawal', compact('details'));
        }
        return view('dashboard.getdetailsaldoawal', compact('details'));
    }
    ...
    public function getdetailmutasi(Request $request)
    {
        ...
        $agent = new Agent();
        if ($agent->isMobile()) {
            return view('dashboard.mobile.getdetailmutasi', compact('details', 'jenis'));
        }
        return view('dashboard.getdetailmutasi', compact('details', 'jenis'));
    }

    public function getdetailsaldoawalkb(Request $request)
    {
        $tanggal = $request->tanggal;
        $details = DB::table('keuangan_saldokasbesar')
            ->join('cabang', 'keuangan_saldokasbesar.kode_cabang', '=', 'cabang.kode_cabang')
            ->where('keuangan_saldokasbesar.tanggal', $tanggal)
        ...
        return view('dashboard.getdetailsaldoawalkb', compact('details'));
    }

    public function getdetailmutasikb(Request $request)
    {
        $tanggal = $request->tanggal;
        $jenis = $request->jenis; // K for Penerimaan, D for Pengeluaran
        $details = DB::table('keuangan_mutasi')
            ...
            ->select('keuangan_mutasi.*', 'keuangan_mutasi_kategori.nama_kategori')
            ->orderBy('keuangan_mutasi.id', 'asc')
            ->get();

        $agent = new Agent();
        if ($agent->isMobile()) {
            return view('dashboard.mobile.getdetailmutasikb', compact('details', 'jenis'));
        }
        return view('dashboard.getdetailmutasikb', compact('details', 'jenis'));
    }
    ...
    public function dashboardowner(Request $request)
    {
        ...
        // Total Saldo Awal Rekening
        $saldo_awal = DB::table('keuangan_rekening_saldoawal_detail')
            ->join('keuangan_rekening_saldoawal', 'keuangan_rekening_saldoawal_detail.kode_saldo_awal', '=', 'keuangan_rekening_saldoawal.kode_saldo_awal')
            ->where('keuangan_rekening_saldoawal.tanggal', $tanggal)
            ->where('keuangan_rekening_saldoawal_detail.kode_bank', '!=', 'BK070')
            ->sum('jumlah');

        // Total Penerimaan dan Pengeluaran dari Mutasi
        $mutasi = DB::table('keuangan_mutasi')
            ->select(
                DB::raw("SUM(IF(debet_kredit='K',jumlah,0)) as penerimaan"),
                DB::raw("SUM(IF(debet_kredit='D',jumlah,0)) as pengeluaran")
            )
            ->where('tanggal', $tanggal)
            ->where('kode_bank', '!=', 'BK070')
        ...
        $data['penerimaan'] = $mutasi->penerimaan ?? 0;
        $data['pengeluaran'] = $mutasi->pengeluaran ?? 0;
        $data['net'] = $saldo_awal + ($mutasi->penerimaan ?? 0) - ($mutasi->pengeluaran ?? 0);
        ...
        $agent = new Agent();
        if ($agent->isMobile()) {
            return view('dashboard.mobile.owner', $data);
        }
    ...
    function owner($request)
    {
        ...
        $agent = new Agent();
        if ($agent->isMobile()) {
            $data['penerimaan'] = $data['rekap']->total_rekap_kredit ?? 0;
            $data['pengeluaran'] = $data['rekap']->total_rekap_debet ?? 0;
            $data['net'] = $data['rekap']->total_saldo ?? 0;
            $data['saldo_awal'] = $data['net'] - $data['penerimaan'] + $data['pengeluaran'];
            $data['tanggal'] = $request->dari ?? date('Y-m-d');
            return view('dashboard.mobile.owner', $data);
        }
    ...
    public function hrd()
    {
        ...
        $data['kontrak_belum_penilaian'] = $sk->getKontrakBelumPenilaian();
