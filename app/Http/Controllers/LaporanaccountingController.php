...
class LaporanaccountingController extends Controller
{
    ...
    public function cetakneracaperbandingan($request)
    {
        $tahun = $request->tahun;
        $kategori = 'NC';

        // Subquery untuk pivot data per bulan dari laporan_keuangan_detail
        $pivot_data = DB::table('laporan_keuangan_detail')
            ->join('laporan_keuangan', 'laporan_keuangan_detail.kode_lk', '=', 'laporan_keuangan.kode_lk')
            ->select(
                'laporan_keuangan_detail.kode_akun',
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 1 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as januari'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 2 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as februari'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 3 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as maret'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 4 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as april'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 5 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as mei'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 6 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as juni'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 7 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as juli'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 8 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as agustus'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 9 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as september'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 10 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as oktober'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 11 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as november'),
                DB::raw('SUM(CASE WHEN laporan_keuangan.bulan = 12 THEN laporan_keuangan_detail.jumlah ELSE 0 END) as desember')
            )
            ->where('laporan_keuangan.tahun', $tahun)
            ->where('laporan_keuangan.kategori', $kategori)
            ->groupBy('laporan_keuangan_detail.kode_akun');

        // Join dengan COA agar struktur neraca tetap terjaga (memunculkan semua akun neraca)
        $neraca_prefixes = ['1', '2', '3'];
        $excluded_accounts = ['0-0000', '1', '2'];

        $data_perbandingan = Coa::leftJoinSub($pivot_data, 'pivot', function ($join) {
            $join->on('coa.kode_akun', '=', 'pivot.kode_akun');
        })
            ->select(
                'coa.kode_akun',
                'coa.nama_akun',
                'coa.level',
                'coa.sub_akun',
                DB::raw('IFNULL(pivot.januari, 0) as januari'),
                DB::raw('IFNULL(pivot.februari, 0) as februari'),
                DB::raw('IFNULL(pivot.maret, 0) as maret'),
                DB::raw('IFNULL(pivot.april, 0) as april'),
                DB::raw('IFNULL(pivot.mei, 0) as mei'),
                DB::raw('IFNULL(pivot.juni, 0) as juni'),
                DB::raw('IFNULL(pivot.juli, 0) as juli'),
                DB::raw('IFNULL(pivot.agustus, 0) as agustus'),
                DB::raw('IFNULL(pivot.september, 0) as september'),
                DB::raw('IFNULL(pivot.oktober, 0) as oktober'),
                DB::raw('IFNULL(pivot.november, 0) as november'),
                DB::raw('IFNULL(pivot.desember, 0) as desember')
            )
        ...
        if (isset($_POST['exportButton'])) {
            header("Content-type: application/vnd-ms-excel");
            header("Content-Disposition: attachment; filename=Neraca Perbandingan $tahun.xls");
        }

        return view('accounting.laporan.lk.neraca_perbandingan', $data);
    }

    public function cetaklabarugiperbandingan($request)
    {
        $tahun = $request->tahun;
        $kategori = 'LB';

        // Subquery untuk pivot data per bulan
        $pivot_data = DB::table('laporan_keuangan_detail')
            ->join('laporan_keuangan', 'laporan_keuangan_detail.kode_lk', '=', 'laporan_keuangan.kode_lk')
            ->select(
                'laporan_keuangan_detail.kode_akun',
        ...
        if (isset($_POST['exportButton'])) {
            header("Content-type: application/vnd-ms-excel");
            header("Content-Disposition: attachment; filename=Laba Rugi Perbandingan $tahun.xls");
        }
    ...
    {
        $tahun = $request->tahun;
        $sampai_bulan = $request->sampai_bulan ?? 12;
        
        // Fetch monthly data for the year up to sampai_bulan (NC and LB)
        $monthly_data = DB::table('laporan_keuangan_detail')
            ...
            ->where('laporan_keuangan.tahun', $tahun)
            ->where('laporan_keuangan.bulan', '<=', $sampai_bulan)
            ->whereIn('laporan_keuangan.kategori', ['NC', 'LB'])
            ->get();

        $ratios = [];
        for ($i = 1; $i <= $sampai_bulan; $i++) {
        ...
        // Calculate Averages
        $avg_current_assets = collect($ratios)->avg('current_assets');
        $avg_current_liabilities = collect($ratios)->avg('current_liabilities');
        $avg_cash_bank = collect($ratios)->avg('cash_bank');
        $avg_total_assets = collect($ratios)->avg('total_assets');
        $avg_total_liabilities = collect($ratios)->avg('total_liabilities');
        $avg_total_equity = collect($ratios)->avg('total_equity');
        $avg_sales = collect($ratios)->avg('sales');
        $avg_cogs = collect($ratios)->avg('cogs');
        $avg_gross_profit = collect($ratios)->avg('gross_profit');
        $avg_net_profit = collect($ratios)->avg('net_profit');

        $averages = [
            'current_assets' => $avg_current_assets,
            'current_liabilities' => $avg_current_liabilities,
            'cash_bank' => $avg_cash_bank,
            'total_assets' => $avg_total_assets,
            'total_liabilities' => $avg_total_liabilities,
            'total_equity' => $avg_total_equity,
            'sales' => $avg_sales,
            'cogs' => $avg_cogs,
            'gross_profit' => $avg_gross_profit,
            'net_profit' => $avg_net_profit,
            'current_ratio' => $avg_current_liabilities != 0 ? ($avg_current_assets / $avg_current_liabilities) * 100 : 0,
            'cash_ratio' => $avg_current_liabilities != 0 ? ($avg_cash_bank / $avg_current_liabilities) * 100 : 0,
            'dar' => $avg_total_assets != 0 ? ($avg_total_liabilities / $avg_total_assets) * 100 : 0,
            'der' => $avg_total_equity != 0 ? ($avg_total_liabilities / $avg_total_equity) * 100 : 0,
            'gross_profit_margin' => $avg_sales != 0 ? ($avg_gross_profit / $avg_sales) * 100 : 0,
            'net_profit_margin' => $avg_sales != 0 ? ($avg_net_profit / $avg_sales) * 100 : 0,
        ];
    ...
    public function lockNeraca(Request $request)
    {
        ...
        $kategori = 'NC';
        $kode_lk = 'LK' . $kategori . $bulan . $tahun;
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Laporan Neraca Berhasil Dikunci!'
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
    ...
    public function lockLabaRugi(Request $request)
    {
        $request->validate([
            'dari' => 'required|date',
            'sampai' => 'required|date',
        ...
        try {
            ...
            return response()->json([
                'success' => true,
                'message' => 'Laporan Laba Rugi Berhasil Dikunci!'
            ]);
        } catch (\Exception $e) {
            DB::rollBack();
            return response()->json([
