            ...
            $query->where(function ($access) use ($user) {
                ...
                if (!in_array('all', $cabang_access)) {
                    if (!empty($cabang_access)) {
                        $access->whereIn('hrd_izinpulang.kode_cabang', $cabang_access);
                    } else {
                        if (empty($user->kode_regional) || $user->kode_regional == 'R00') {
                            $access->where('hrd_izinpulang.kode_cabang', $user->kode_cabang);
                        }
                    }
                }
                ...
                if (!in_array('all', $dept_access)) {
                    $access->whereIn('hrd_izinpulang.kode_dept', $dept_access);
                }

                // Jabatan Access
                if (!in_array('all', $jabatan_access)) {
                    $access->whereIn('hrd_izinpulang.kode_jabatan', $jabatan_access);
                }
