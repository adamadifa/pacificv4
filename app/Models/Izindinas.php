...
class Izindinas extends Model
{
    ...
    function getIzindinas($kode_izin_dinas = null, Request $request = null)
    {
        ...
        $query->leftJoin('hrd_izindinas_disposisi as disposisi', function ($join) {
            $join->on('hrd_izindinas.kode_izin_dinas', '=', 'disposisi.kode_izin_dinas')
                ->whereRaw('disposisi.kode_disposisi IN (SELECT MAX(kode_disposisi) FROM hrd_izindinas_disposisi GROUP BY kode_izin_dinas)');
        });
        ...
        if (!in_array($role, ['super admin', 'asst. manager hrd', 'spv presensi', 'direktur'])) {
            if ($user->hasRole('gm operasional')) {
                $query->whereIn('hrd_izindinas.kode_dept', ['PDQ', 'PMB', 'GDG', 'MTC', 'PRD', 'GAF', 'HRD']);
                $query->whereIn('hrd_izindinas.kode_jabatan', ['J05', 'J06']);
            } else if ($user->hasRole('gm administrasi')) { //GM ADMINISTRASI
                $query->whereIn('hrd_izindinas.kode_dept', ['AKT', 'KEU']);
                $query->whereIn('hrd_izindinas.kode_jabatan', ['J04', 'J05', 'J06']);
            } elseif ($user->hasRole('gm marketing')) { //GM MARKETING
                $query->whereIn('hrd_izindinas.kode_dept', ['MKT']);
                $query->whereIn('hrd_izindinas.kode_jabatan', ['J03', 'J05', 'J06']);
            } else if ($user->hasRole('regional sales manager')) { //REG. SALES MANAGER
                $query->where('hrd_izindinas.kode_dept', 'MKT');
                $query->where('hrd_izindinas.kode_jabatan', 'J07');
                ...
                $query->whereIn('hrd_izindinas.kode_jabatan', ['J08', 'J12', 'J13', 'J14']);
            } else {
                if (auth()->user()->kode_cabang == 'PST') {
                    $query->where('hrd_izindinas.kode_dept', auth()->user()->kode_dept);
            ...
            if ($user->hasRole('gm operasional')) {
                ...
                $query->orwhere('hrd_izindinas.kode_dept', 'MKT');
                $query->where('hrd_izindinas.kode_jabatan', 'J07');
                $query->where('cabang.kode_regional', auth()->user()->kode_regional);
            } else if ($user->hasRole('regional operation manager')) { //REG. OPERATION MANAGER
                ...
                if (auth()->user()->kode_cabang == 'PST') {
                    $query->orwhere('hrd_izindinas.kode_dept', auth()->user()->kode_dept);
                    $query->where('hrd_izindinas.kode_cabang', auth()->user()->kode_cabang);
                } else {
            ...
            $query->WhereIn('hrd_izindinas.kode_izin_dinas', function ($query) use ($user) {
                ...
                $query->from('hrd_izindinas_disposisi as disposisi');
