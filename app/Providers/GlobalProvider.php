        ...
        view()->composer('*', function ($view) use ($auth) {
            ...
            if ($auth->check()) {
                ...
                $notifikasi_izinabsen_presensi = Disposisiizinabsen::where('id_penerima', auth()->user()->id)->where('hrd_izinabsen_disposisi.status', 0)
                    ...
                    ->count();
                $notifikasi_izincuti_presensi = Disposisiizincuti::where('id_penerima', auth()->user()->id)->where('hrd_izincuti_disposisi.status', 0)
                    ->leftJoin('users', 'hrd_izincuti_disposisi.id_penerima', '=', 'users.id')
                    ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
                    ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->where('roles.name', 'asst. manager hrd')
                    ->count();
                $notifikasi_izinterlambat_presensi = Disposisiizinterlambat::where('id_penerima', auth()->user()->id)->where('hrd_izinterlambat_disposisi.status', 0)
                    ->leftJoin('users', 'hrd_izinterlambat_disposisi.id_penerima', '=', 'users.id')
                    ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
                    ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->where('roles.name', 'asst. manager hrd')
                    ->count();
                $notifikasi_izinsakit_presensi = Disposisiizinsakit::where('id_penerima', auth()->user()->id)->where('hrd_izinsakit_disposisi.status', 0)
                    ->leftJoin('users', 'hrd_izinsakit_disposisi.id_penerima', '=', 'users.id')
                    ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
                    ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->where('roles.name', 'asst. manager hrd')
                    ->count();
                $notifikasi_izinpulang_presensi = Disposisiizinpulang::where('id_penerima', auth()->user()->id)->where('hrd_izinpulang_disposisi.status', 0)
                    ->leftJoin('users', 'hrd_izinpulang_disposisi.id_penerima', '=', 'users.id')
                    ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
                    ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->where('roles.name', 'asst. manager hrd')
                    ->count();
                $notifikasi_izindinas_presensi = Disposisiizindinas::where('id_penerima', auth()->user()->id)->where('hrd_izindinas_disposisi.status', 0)
                    ->leftJoin('users', 'hrd_izindinas_disposisi.id_penerima', '=', 'users.id')
                    ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
                    ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->where('roles.name', 'asst. manager hrd')
                    ->count();
                $notifikasi_izinkoreksi_presensi = Disposisiizinkoreksi::where('id_penerima', auth()->user()->id)->where('hrd_izinkoreksi_disposisi.status', 0)
                    ->leftJoin('users', 'hrd_izinkoreksi_disposisi.id_penerima', '=', 'users.id')
                    ->leftJoin('model_has_roles', 'users.id', '=', 'model_has_roles.model_id')
                    ->leftJoin('roles', 'model_has_roles.role_id', '=', 'roles.id')
                    ->where('roles.name', 'asst. manager hrd')
                    ->count();
                $notifikasi_izinkeluar_presensi = Disposisiizinkeluar::where('id_penerima', auth()->user()->id)->where('hrd_izinkeluar_disposisi.status', 0)
